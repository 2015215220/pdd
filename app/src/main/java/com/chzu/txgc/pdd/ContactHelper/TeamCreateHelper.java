package com.chzu.txgc.pdd.ContactHelper;

import android.content.Context;
import android.os.Handler;
import android.util.Log;
import com.chzu.txgc.pdd.Contact.DemoCache;
...
public class TeamCreateHelper {
        ...
        NIMClient.getService(TeamService.class).createTeam(fields, TeamTypeEnum.Normal, "",
                ...
                new RequestCallback<CreateTeamResult>() {
                    ...
                    public void onSuccess(CreateTeamResult result) {
                        ...
                        if (isNeedBack) {
                            NimUIKitImpl.startTeamSession(context, result.getTeam().getId(), null); // 进入创建的群
                        } else {
                            NimUIKitImpl.startTeamSession(context, result.getTeam().getId());
                        }
                        if (callback != null) {
    ...
    private static void onCreateSuccess(final Context context, CreateTeamResult result) {
        ...
        new Handler(context.getMainLooper()).postDelayed(new Runnable() {
            @Override
            public void run() {
                NimUIKitImpl.startTeamSession(context, team.getId()); // 进入创建的群
            }
        }, 50);
    }
}
